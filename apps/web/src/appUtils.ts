  ...
  try {
    ...
    if (parsed.llama_runtime) {
      ...
      parsed.llama_runtime.gpu_driver_process_scope_summary ??=
        "This older report did not classify whether NVML driver APIs saw the runtime in compute-process scope, graphics-process scope, both, or neither.";
      parsed.llama_runtime.gpu_evidence_tier_status ??=
        "gpu_evidence_tier_unavailable_in_legacy_report";
      parsed.llama_runtime.gpu_evidence_tier_summary ??=
        "This older report did not collapse its GPU evidence into a stable Windows/NVIDIA evidence tier.";
      parsed.llama_runtime.gpu_claim_boundary_status ??=
      ...
      parsed.llama_runtime.gpu_allocator_visibility_status ??=
        "gpu_allocator_visibility_unavailable_in_legacy_report";
      parsed.llama_runtime.gpu_allocator_visibility_summary ??=
        "This older report did not state how far its GPU evidence reached toward allocator ownership, released-allocation truth, or per-buffer attribution.";
    ...
    if (parsed.memory_validation?.stage_scorecards) {
      for (const scorecard of parsed.memory_validation.stage_scorecards) {
        ...
        if (scorecard.cleanup_signal_support_scope_status === undefined) {
          scorecard.cleanup_signal_support_scope_status = "cleanup_signal_scope_unavailable";
        }
        if (scorecard.cleanup_signal_support_scope_summary === undefined) {
          scorecard.cleanup_signal_support_scope_summary =
            "This older report did not distinguish whether allocator/KV cleanup evidence for the validation scorecard was stage-local or only runtime-global.";
        }
        if (scorecard.contributing_cleanup_signals === undefined) {
          scorecard.contributing_cleanup_signals = [];
        }
      }
    }
